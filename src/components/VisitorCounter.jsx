...
function VisitorCounter({ variant = "card" }) {
  ...
  useEffect(() => {
    ...
    const externalUrl = "https://api.counterapi.dev/v2/daniel-garcias-team-4475/first-counter-4475";
    const url = isDev ? "/api/counter" : externalUrl;
    ...
    fetch(url, fetchOptions)
      .then((response) => {
        if (!response.ok) {
          throw new Error("Error en la petición al contador");
        }
        return response.json();
      })
      .then((data) => {
        // La API v2 responde { code, data: { up_count, ... } }
